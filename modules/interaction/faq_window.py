...
async def faq_window(msg: types.Message, state: FSMContext = None) -> None:
    ...
    await msg.bot.send_message(msg.from_user.id,
                               """В данном разделе представлены часто задаваемые вопросы.\n{}\nВам помог этот раздел?""".format(
                                   '\n'.join(list(
                                       f"{i} - {questions[i - 1]['question']}: {questions[i - 1]['answer']}" for i in
                                       range(1, len(questions) + 1)))) if len(questions) > 0 else "Тут ничего нет",
...
async def faq_question(msg: types.Message, state: FSMContext) -> None:
    if msg.text.startswith('Да'):
        ...
        await msg.bot.send_message(msg.chat.id, "Вы можете отправить Ваш запрос командой /submit_request")
    await state.clear()
